...
function EditModalForm({ loanData, editModal, closeEditModal, token, setLoanData }) {
    ...
    function postBorrowerName() {
        axios
            .post(url + '/loan/postBorrowerName', {
                'newBorrowerName': newBorrowerName
            }, {
                headers: { 'authorization': `Bearer ${token}` }
            })
            .then((res) => {
                setLoanData({ ...loanData, borrowerList: res.data.borrowerList })
                setNameForm(false)
            })
            .catch((error) => {
                console.log(error)
    ...
    function validateForm() {
        ...
        let duration = moment(formData.endDate).diff(moment(formData.startDate), 'days')
        if (duration < 0) {
    ...
    function submitAddLoan() {
        ...
        if (!valid) return
        axios
            .post(url + '/loan/postBorrowerLoan', formData, {
                headers: { 'authorization': `Bearer ${token}` }
            })
            .then(() => {
                alert("Loan Added")
    ...
    function submitEditLoan() {
        ...
        if (!valid) return
        axios
            .put(url + '/loan/editBorrowerLoan', formData, {
                headers: { 'authorization': `Bearer ${token}` }
            })
            .then(() => {
                alert("Loan Edited")
                    ...
                    <Row>
                        <Col md={12} className='mb-3'>
                            ...
                            <div className="container_fluid">
                                <div className="row">
                                    {loanData.borrowerList?.length > 0 ?
                                        loanData.borrowerList.map((name, i) =>
                                    ...
                                    <div className="col-2">
                                        <div className="name_box add_name mb-3" onClick={toggleNameForm}>
                                            {!nameForm ?
                                                <><FontAwesomeIcon icon={faPlus} /> Add Borrower</> :
                        ...
                        {nameForm ?
                            <Col md={12} className='mb-3'>
                                <Label htmlFor="name">Enter A New Borrower Name</Label>
                                <Input type="text" id="name" name="loanAmount" placeholder="Borrower Name" onChange={e => newBorrowerName = e.target.value} />
                                <div id="error_amount" className='form_error'></div>
                                <div className="btn_cont mt-3"><div className="btn_ btn_small" onClick={postBorrowerName}>Add</div></div>
                            </Col> : <></>
                        }
...
export default function BorrowerDetails() {
        ...
        if (token) {
            try {
                axios
                    ...
                    .then((res) => {
                        setLoanData(res.data.borrowerDetails)
    ...
    function deleteDebt() {
        axios
            .delete(url + '/loan/deleteBorrowerLoan/' + debtDeleteId, {
                headers: { 'authorization': `Bearer ${token}` }
            })
            .then((res) => {
