...
export default function StockDetails() {
    ...
    const technicalPointers = [
        { title: 'Share Price', value: stockDetail?.price?.regularMarketPrice.raw, icon: <FundOutlined /> },
        { title: 'Market Cap', value: stockDetail?.summaryDetail?.marketCap.raw, icon: <ExclamationCircleOutlined /> },
        { title: '52 Week High', value: stockDetail?.summaryDetail?.fiftyTwoWeekHigh.raw, icon: <ExclamationCircleOutlined /> },
        { title: '52 Week Low', value: stockDetail?.summaryDetail?.fiftyTwoWeekLow.raw, icon: <ExclamationCircleOutlined /> },
        { title: 'Total Volume', value: stockDetail?.summaryDetail?.volume.raw, icon: <ExclamationCircleOutlined /> },
        { title: 'Avg. 10 Day Daily Volume', value: stockDetail?.price?.averageDailyVolume10Day.raw, icon: <MoneyCollectOutlined /> },
        { title: 'Avg. 3 Month Daily Volume', value: stockDetail?.price?.averageDailyVolume3Month.raw, icon: <ExclamationCircleOutlined /> },
    ]

    const otherStats = [
        { title: 'Sector Type', value: stockDetail?.assetProfile?.sector, icon: <ExclamationCircleOutlined /> },
        { title: 'Industry', value: stockDetail?.assetProfile?.industry, icon: <ExclamationCircleOutlined /> },
        { title: 'Exchange Name', value: stockDetail?.price?.exchangeName, icon: <ExclamationCircleOutlined /> },
        { title: 'Revenue High', value: stockDetail?.calendarEvents?.earnings?.revenueHigh.raw, icon: <ExclamationCircleOutlined /> },
        { title: 'Revenue Low', value: stockDetail?.calendarEvents?.earnings?.revenueLow.raw, icon: <ExclamationCircleOutlined /> },
        { title: 'Revenue Avg.', value: stockDetail?.calendarEvents?.earnings?.revenueAverage.raw, icon: <ExclamationCircleOutlined /> },
        { title: 'Dividend (Yield)', value: (`${stockDetail?.summaryDetail?.dividendRate.raw} (${stockDetail?.summaryDetail?.dividendYield.raw?.toFixed(6) * 100}%)`), icon: <ExclamationCircleOutlined /> }
    ]
    ...
    function getStockHistory(timeperiod){
        ...
        return (
            <div className="container stock_detail pt-5 pb-5">
                ...
                <LineChart stockHistory={stockHistory} stockName={stockDetail?.quoteType?.shortName} timeperiod={timeperiod} isFetching={isFetching} />
                ...
                <div className="row stock_info mt-5">
                    ...
                    <div className="company_details">
                        ...
                        <div className="detail mt-2"><span>Address: </span>{stockDetail?.assetProfile?.address1}, {stockDetail?.assetProfile?.address2}, {stockDetail?.assetProfile?.city} - {stockDetail?.assetProfile?.zip}, {stockDetail?.assetProfile?.state}, {stockDetail?.assetProfile?.country} </div>
